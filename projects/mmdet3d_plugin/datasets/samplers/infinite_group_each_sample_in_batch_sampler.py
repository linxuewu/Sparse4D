...
class InfiniteGroupEachSampleInBatchSampler(Sampler):
            ...
            for local_sample_idx in range(self.batch_size):
                ...
                if len(self.buffer_per_local_sample[local_sample_idx]) == 0:
                    ...
                    if self.dataset.keep_consistent_seq_aug:
                        self.aug_per_local_sample[local_sample_idx] = self.get_aug()
    ...
    def get_aug(self):
        rot_angle = np.random.uniform(*self.dataset.rot_range)
        scale_ratio = np.random.uniform(*self.dataset.scale_ratio_range)
        aug_configs = self.dataset._sample_augmentation()
        return rot_angle, scale_ratio, aug_configs
